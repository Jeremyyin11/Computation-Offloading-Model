...
int main () {
	map<string, string > serverA_map;

	ifstream myFile("database_a.csv");
	if (!myFile.is_open()) {
	...
	while (myFile.good()) {
	    ...
	    serverA_map.insert(pair <string, string> (LindId, str)); 
	}
	myFile.close(); 
	...
	printf("The ServerA received input <%s>\n", buf);
	string sendbuf;
	if(serverA_map.find(buf) != serverA_map.end()){
		printf("The server A has found <1> match\n");
		sendbuf = serverA_map[buf];
	} else {
